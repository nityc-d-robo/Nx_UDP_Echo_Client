...
TX_THREAD AppFDCANThread;

NX_UDP_SOCKET UDPSocket;
ULONG IpAddress;
ULONG NetMask;

#define FDCAN_PRIORITY 12
#define FDCAN_THREAD_STACK_SIZE 1024
uint8_t fdcan_thread_stack[FDCAN_THREAD_STACK_SIZE];
...
static VOID App_FDCAN_Thread_Entry(ULONG thread_input);
...
UINT MX_NetXDuo_Init(VOID *memory_ptr)
{
  ...
  // Create FDCAN receive thread
  ret = tx_thread_create(&AppFDCANThread, "FDCAN Thread", App_FDCAN_Thread_Entry, 0, fdcan_thread_stack, FDCAN_THREAD_STACK_SIZE,
		  	  	  	  	    FDCAN_PRIORITY, FDCAN_PRIORITY, TX_NO_TIME_SLICE, TX_DONT_START);
...
static VOID nx_app_thread_entry (ULONG thread_input)
{
  ...
  /* start the FDCAN thread */
  tx_thread_resume(&AppFDCANThread);
...
static VOID App_UDP_Thread_Entry(ULONG thread_input)
{
  ...
  while(1)
  {
    ...
    if (ret == NX_SUCCESS)
    {
      ...
      uint8_t TxData[8] = {(8000 >> 8) & 0xff, 8000 & 0xff, 0};
...
static VOID App_FDCAN_Thread_Entry(ULONG thread_input){
    ...
    while (1)
	{
	    ...
	    {
	        /* 受信データの処理 */
	    	printf("Received CAN Packet. {Angle: %d, Speed: %d, Torque: %d}\n",
	    			(RxData[0] << 8) | RxData[1], (RxData[2] << 8) | RxData[3], (RxData[4] << 8) | RxData[5]
			);
	    }
